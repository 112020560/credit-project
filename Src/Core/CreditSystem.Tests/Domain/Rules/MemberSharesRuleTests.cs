...
public class MemberSharesRuleTests
{
    ...
    public async Task EvaluateAsync_WhenAmountWithinLimit_ShouldPass()
    {
        ...
        var rule = new MemberSharesRule(DefaultPolicy(multiplier: 5));
        var context = BuildContext(requestedAmount: 10_000m, sharesAmount: 5_000m);
    ...
    public async Task EvaluateAsync_WhenAmountExceedsLimit_ShouldFail()
    {
        ...
        var rule = new MemberSharesRule(DefaultPolicy(multiplier: 5));
        var context = BuildContext(requestedAmount: 30_000m, sharesAmount: 5_000m);
    ...
    {
        var rule = new MemberSharesRule(DefaultPolicy());
    ...
    {
        var rule = new MemberSharesRule(DefaultPolicy(requireMembership: true));
        var context = BuildContext(isActiveMember: false, sharesAmount: 50_000m);
    ...
    {
        var rule = new MemberSharesRule(DefaultPolicy(requireMembership: false));
        var context = BuildContext(isActiveMember: false, sharesAmount: 5_000m, requestedAmount: 10_000m);
